...
ALPHA_BLEND  = 0.6     # STEALTH: patch opacity (car shows through -> looks like reflection/lighting)
COLOR_WEIGHT = 0.15    # STEALTH: pull patch toward muted car-like colors (camouflage)
...
def apply_patch(img, patch, boxes_norm):
    ...
    for (xc, yc, bw, bh) in boxes_norm:
        ...
        # STEALTH: within the tilted mask, blend patch semi-transparently with the car
        blended = ALPHA_BLEND * p_rot + (1 - ALPHA_BLEND) * region
        out[0, :, y1:y2, x1:x2] = m_rot * blended + (1 - m_rot) * region
    return out
...
def color_loss(patch):
    """STEALTH camouflage: pull patch toward muted, car-like colors (low saturation)
    so it blends into the vehicle instead of looking like a rainbow sticker."""
    mean_c = patch.mean(dim=0, keepdim=True)
    saturation = (patch - mean_c).abs().mean()
    brightness_pen = (patch.mean() - 0.45).abs()
    return saturation + 0.3 * brightness_pen
...
def run():
    ...
    for ep in range(EPOCHS):
        ...
        for stem, x0, boxes in data:
            ...
            loss = targeted_loss(raw(patched), boxes) + TV_WEIGHT * tv_loss(patch) + COLOR_WEIGHT * color_loss(patch)
