...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_ms"   # overridden
...
PATCH_PX = 160
PAINT_METHOD = "brightness"  # base method (overridden by COMBINE_METHOD)
COMBINE_METHOD = "adaptive"  # adaptive | union | intersection | vote
PAINT_TILES  = 49            # tiles when USE_PAINTTILES
MS_SCALES    = 3             # number of multiscale layers (the R63 winner)
MS_RATIO     = 2.0           # scale factor between layers (2.0 => full, 1/2, 1/4)
COVER_FLOOR  = 0.40          # GUARANTEE: at least this fraction of each car gets painted
PAINT_TILE_INDEP = True      # independent (each tile its own pattern) vs shared
...
def _mask_brightness(crop_bgr, valid=None):
    import numpy as _np, cv2 as _cv2
    g=_cv2.cvtColor(crop_bgr,_cv2.COLOR_BGR2GRAY).astype(_np.float32)
    # PER-BOX: compute the brightness cutoff over ONLY the car's pixels (valid mask),
    # so surrounding pavement/other cars don't skew the percentile.
    if valid is not None and valid.sum() > 10:
        lo=_np.percentile(g[valid>0.5], PAINT_LO_PCT)
    else:
...
def _mask_domcolor(crop_bgr, valid=None):
    import numpy as _np, cv2 as _cv2
    # PER-BOX: cluster ONLY the car's pixels so pavement doesn't form a cluster
    if valid is not None and valid.sum() > 20:
        pix = crop_bgr[valid>0.5].astype(_np.float32)
    else:
        pix = crop_bgr.reshape(-1,3).astype(_np.float32)
    small = pix
    K=4; crit=(_cv2.TERM_CRITERIA_EPS+_cv2.TERM_CRITERIA_MAX_ITER,10,1.0)
    try:
        _,labels,centers=_cv2.kmeans(small,K,None,crit,3,_cv2.KMEANS_PP_CENTERS)
    except Exception:
        return _np.ones(crop_bgr.shape[:2],_np.float32)
    labels=labels.ravel()
    bright=[centers[k].mean() for k in range(K)]
    sizes=[(labels==k).sum() for k in range(K)]
    order=_np.argsort(bright); bc=order[K//2:]
    paint_k=int(bc[_np.argmax([sizes[k] for k in bc])])
    # map the chosen cluster back onto the 2D crop
    outm=_np.zeros(crop_bgr.shape[:2],_np.float32)
    if valid is not None and valid.sum()>20:
        ys,xs=_np.nonzero(valid>0.5)
        sel=(labels==paint_k)
        outm[ys[sel],xs[sel]]=1.0
    else:
        outm=(labels.reshape(crop_bgr.shape[:2])==paint_k).astype(_np.float32)
    return outm

# --- coverage instrumentation (measurement only; does NOT alter any mask) ---
_COV = {"frac_sum":0.0,"n":0,"loosened":0,"fallback":0}

def _apply_cover_floor(m, valid, crop_bgr):
    """Guarantee at least COVER_FLOOR of the car's area is painted. If the detected
    mask is too small (e.g. uniform-coloured car where the percentile lands badly),
    progressively loosen a brightness cutoff until the floor is met."""
    import numpy as _np, cv2 as _cv2
    if valid is None or valid.sum() < 10: return m
    car_px = valid.sum()
    if (m*valid).sum() >= COVER_FLOOR*car_px: return m       # already enough
    g = _cv2.cvtColor(crop_bgr,_cv2.COLOR_BGR2GRAY).astype(_np.float32)
    gv = g[valid>0.5]
    # loosen: try progressively lower percentiles until the floor is satisfied
    for pct in (30, 20, 12, 6, 2, 0):
        lo = _np.percentile(gv, pct)
        cand = ((g > lo).astype(_np.float32)) * valid
        if cand.sum() >= COVER_FLOOR*car_px:
            _COV["loosened"] += 1
            return cand
    _COV["fallback"] += 1
    return valid.copy()    # last resort: paint the whole car footprint

def _detect_paint_mask(crop_bgr, valid=None):
    """Combine brightness + dom-color per COMBINE_METHOD, computing statistics ONLY over
    the car's pixels (valid mask) so surrounding ground doesn't contaminate detection."""
    import numpy as _np, cv2 as _cv2
    if crop_bgr.shape[0]<3 or crop_bgr.shape[1]<3:
        return _np.ones(crop_bgr.shape[:2],_np.float32)
    mb=_mask_brightness(crop_bgr, valid); md=_mask_domcolor(crop_bgr, valid)
    if COMBINE_METHOD=="adaptive":
        # PER-BOX: judge car darkness from the CAR's pixels only
        g=_cv2.cvtColor(crop_bgr,_cv2.COLOR_BGR2GRAY)
        avg = g[valid>0.5].mean() if (valid is not None and valid.sum()>10) else g.mean()
        m = mb if avg>=110 else md
    elif COMBINE_METHOD=="union":
        m=_np.maximum(mb,md)
    elif COMBINE_METHOD=="intersection":
        m=mb*md
    else:  # vote / average
        m=((mb+md)*0.5>=0.5).astype(_np.float32)
    # HARDEN: erode the mask so paint stays strictly INSIDE the detected car region
    # (removes thin spurs that reach ground/edges), then keep only the largest blob.
    k=_np.ones((3,3),_np.uint8)
    m=_cv2.erode(m.astype(_np.uint8),k,iterations=1).astype(_np.float32)
    # keep largest connected component (the car body), drop stray ground specks
    n,lab=_cv2.connectedComponents(m.astype(_np.uint8))
    if n>2:
        sizes=[(lab==i).sum() for i in range(1,n)]
        big=1+int(_np.argmax(sizes))
        m=(lab==big).astype(_np.float32)
    # COVERAGE FLOOR: never leave a car (nearly) unpainted
    m=_apply_cover_floor(m, valid, crop_bgr)
    return m
...
def paint_apply(out, img, patch, xc, yc, bw, bh, angle):
    ...
    # ANGLE-FREE inscribed ellipse: largest ellipse fitting inside the axis-aligned
    # VisDrone box, 0.98 safety margin. No orientation term -> no diagonal strips.
    # PER-BOX: crop TIGHT to the ground-truth box (not the diagonal), so the detector
    # sees mostly car pixels instead of surrounding pavement/neighbours.
    x1=max(0,int(cx-box_w_px/2)); y1=max(0,int(cy-box_h_px/2))
    x2=min(W,int(cx+box_w_px/2)); y2=min(H,int(cy+box_h_px/2))
    rh,rw=y2-y1,x2-x1
    if rh<4 or rw<4: return
    # DETECT car paint (per-box, car pixels only)
    ...
    ax=box_w_px/2*0.98; ay=box_h_px/2*0.98
    inside=((((xs-cx)/ax)**2 + ((ys-cy)/ay)**2) <= 1.0).astype('float32')
    paint = _detect_paint_mask(crop_bgr, valid=inside)
    mask = paint * inside
    _COV["frac_sum"] += float(mask.sum()) / max(1.0, float(inside.sum()))   # instrumentation
    _COV["n"] += 1                                                          # instrumentation
    cmask=_t.from_numpy(mask).to(out.device).unsqueeze(0)
    region=out[0,:,y1:y2,x1:x2]

    # MULTISCALE: build the effective pattern from MS_SCALES layers at different
    # resolutions (the R63 mechanism: fine + medium + coarse frequency content).
    def _ms_pattern(pt):
        import torch as _t
        acc=0.0
        n=pt.shape[0]
        for li in range(n):
            sc = MS_RATIO ** li
            ph2 = max(2, int(PATCH_PX / sc))
            lo=_t.nn.functional.interpolate(pt[li:li+1], size=(ph2,ph2), mode='bilinear', align_corners=False)
            up=_t.nn.functional.interpolate(lo, size=(PATCH_PX,PATCH_PX), mode='bilinear', align_corners=False)[0]
            acc = acc + up
        return (acc/n).clamp(0,1)

    if globals().get('USE_MS', False) and patch.dim()==4 and not globals().get('USE_PAINTTILES', False):
        psrc=_ms_pattern(patch)
        p=_rotate_patch(psrc, 0.0, rh, rw)
        out[0,:,y1:y2,x1:x2]=cmask*p+(1-cmask)*region
        return out

    if globals().get('USE_PAINTTILES', False) and patch.dim()==4:
        # FILL the region with PAINT_TILES independent tiles in a grid, each its own pattern.
        # Then mask the whole tiled fill to the detected paint -> tiles only show on paint.
        import math as _m
        g = int(_m.ceil(_m.sqrt(PAINT_TILES)))
        tiled = region.clone()
        ti = 0
        for r in range(g):
            for c in range(g):
                if ti >= PAINT_TILES: break
                ty1 = int(r*rh/g); ty2 = int((r+1)*rh/g)
                tx1 = int(c*rw/g); tx2 = int((c+1)*rw/g)
                th2, tw2 = ty2-ty1, tx2-tx1
                if th2<2 or tw2<2:
                    ti+=1; continue
                if globals().get('USE_MS', False) and patch.shape[0] >= PAINT_TILES*MS_SCALES:
                    # each tile gets its own MS_SCALES layers -> multiscale per tile
                    grp = patch[ti*MS_SCALES:(ti+1)*MS_SCALES]
                    src = _ms_pattern(grp)
                else:
                    src = patch[ti % patch.shape[0]] if PAINT_TILE_INDEP else patch[0]
                tp = _rotate_patch(src, 0.0, th2, tw2)
                tiled[:, ty1:ty2, tx1:tx2] = tp
                ti += 1
        out[0,:,y1:y2,x1:x2] = cmask*tiled + (1-cmask)*region
    else:
...
def run():
    ...
    if globals().get('USE_PAINTTILES', False) and globals().get('USE_MS', False):
        patch = torch.rand(PAINT_TILES*MS_SCALES, 3, PATCH_PX, PATCH_PX, device=DEVICE, requires_grad=True)
    elif globals().get('USE_PAINTTILES', False):
        patch = torch.rand(PAINT_TILES, 3, PATCH_PX, PATCH_PX, device=DEVICE, requires_grad=True)
    elif globals().get('USE_MS', False):
        patch = torch.rand(MS_SCALES, 3, PATCH_PX, PATCH_PX, device=DEVICE, requires_grad=True)
    else:
    ...
    _COV.update(frac_sum=0.0, n=0, loosened=0, fallback=0)   # reset per run; count eval pass only
    for stem, x0, boxes in data:
        if not boxes: continue
    ...
    print(f"COVERAGE: mean_painted_frac={_COV['frac_sum']/max(1,_COV['n']):.3f} "
          f"loosened={_COV['loosened']} fallback={_COV['fallback']} n_cars={_COV['n']}")
