...
def apply_patch(img, patch, boxes_norm):
    ...
    for (xc, yc, bw, bh) in boxes_norm:
        ...
        # STEALTH: blend the (rotated-texture) patch semi-transparently with the car
        region = out[0, :, y1:y2, x1:x2]
        out[0, :, y1:y2, x1:x2] = ALPHA_BLEND * p + (1 - ALPHA_BLEND) * region
    return out
