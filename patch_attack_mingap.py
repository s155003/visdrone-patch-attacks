...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_mingap"   # overridden per-variant
...
PATTERN_MODE = "independent"  # ALWAYS independent for this batch
PLACEMENT    = "grid"      # grid base for separated layouts
# ---- SEPARATION config (set by orchestrator) ----
SEP_MODE   = "mingap"   # "mingap" = tiles fill cell minus a fixed MIN_GAP_PX (hairline separation)
MIN_GAP_PX = 2         # guaranteed pixel gap between adjacent tiles (never merge)
CELL_FILL  = 0.6        # tile occupies this fraction of its grid cell -> guarantees gaps
BAR_ROWS   = 7          # for SEP_MODE="bars": number of horizontal bars
BAR_FILL_H = 0.5        # each bar's height as fraction of its row slot (rest = gap)
...
def apply_patch(img, patch, boxes_norm):
    ...
    for bi, (xc, yc, bw, bh) in enumerate(boxes_norm):
        ...
        bx1 = int((xc - bw/2)*W); by1 = int((yc - bh/2)*H)
        angle = estimate_car_angle(img, xc, yc, bw, bh, H, W)

        # ---- BARS mode: N horizontal bars with GUARANTEED vertical gaps ----
        if SEP_MODE == "bars":
            rows = BAR_ROWS
            row_h = box_h_px / rows                       # each bar's vertical slot
            bar_h = max(2, int(row_h * BAR_FILL_H))       # bar height (rest is gap)
            bar_w = max(2, int(box_w_px * 0.9))           # bars span most of the width
            for ri in range(rows):
                # bar sits at the TOP of its row slot; gap below guarantees no merge
                ty = int(by1 + ri*row_h)
                y1 = max(0, ty); y2 = min(H, y1 + bar_h)
                x1 = max(0, bx1 + int(box_w_px*0.05)); x2 = min(W, x1 + bar_w)
                th, tw = y2-y1, x2-x1
                if th < 2 or tw < 2: continue
                tile_src = patch[ri % patch.shape[0]]
                p = _rotate_patch(tile_src, angle, th, tw)
                out[0, :, y1:y2, x1:x2] = p
            continue

        # ---- GRID-based separated tiles (spaced / shrunk / both) ----
        g = int(_np.ceil(_np.sqrt(N_TILES)))              # grid dimension
        cell_w = box_w_px / g; cell_h = box_h_px / g      # each tile's cell
        # fill fraction determines the gap: smaller fill = bigger guaranteed gap
        if   SEP_MODE == "mingap":
            # tiles as LARGE as possible: fill the whole cell MINUS a fixed pixel gap
            # -> hairline separation, never touching, closest to contiguous
            tw_t = max(2, int(cell_w) - MIN_GAP_PX)
            th_t = max(2, int(cell_h) - MIN_GAP_PX)
        else:
            if   SEP_MODE == "shrunk": fill = 0.5
            elif SEP_MODE == "both":   fill = 0.4
            else:                       fill = CELL_FILL       # "spaced" default 0.6
            tw_t = max(2, int(cell_w * fill)); th_t = max(2, int(cell_h * fill))
        ti = 0
        for r in range(g):
            for c in range(g):
                if ti >= N_TILES: break
                # center tile within its cell -> gap on all sides guaranteed
                ccx = bx1 + (c + 0.5)*cell_w
                ccy = by1 + (r + 0.5)*cell_h
                x1 = max(0, int(ccx - tw_t/2)); y1 = max(0, int(ccy - th_t/2))
                x2 = min(W, x1 + tw_t);         y2 = min(H, y1 + th_t)
                th2, tw2 = y2-y1, x2-x1
                if th2 >= 2 and tw2 >= 2:
                    tile_src = patch[ti % patch.shape[0]]
                    p = _rotate_patch(tile_src, angle, th2, tw2)
                    out[0, :, y1:y2, x1:x2] = p
                ti += 1
    return out
