...
PATCH_PX = 160                # patch is stored at this resolution, resized per object
# =================================================
...
def paint_apply(out, img, patch, xc, yc, bw, bh, angle):
    ...
    # car-aligned coords: u along length (0..1), v across width (-0.5..0.5)
    uu=((xs-cx)*dx+(ys-cy)*dy)/max(1.0,car_len) + 0.5
    vv=(-(xs-cx)*dy+(ys-cy)*dx)/max(1.0,car_wid)
    inside=(uu>=0.0)&(uu<=1.0)&(_np.abs(vv)<=0.5)   # the inscribed oriented car-rect
    body=_paint_body_mask(uu, vv)
    mask=(inside.astype('float32'))*body
