...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_scaleup"   # overridden per-variant

# ---- patch / optimization params ----
PATCH_FRAC = 0.45      # (unused when FIXED_FRAC>0)
FIXED_FRAC = 0.0       # if >0, every car uses this fixed fraction (overrides adaptive)
...
def run():
    ...
    pimg = (final_patch.permute(1,2,0).cpu().numpy()*255).astype(np.uint8)[:,:,::-1]
