...
NUM_IMAGES = 30
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_oriented_power"
...
CONF_WEIGHT = 1.0      # POWER: also drive up the target class's ABSOLUTE confidence
                       #        (so flipped detections are strong enough to survive NMS/conf-threshold)
MOMENTUM    = 0.9      # POWER: momentum on the patch gradient (MI-style, escapes local optima)
...
def run():
    ...
    for ep in range(EPOCHS):
        ...
        for stem, x0, boxes in data:
            ...
            # POWER: blend this step's gradient with accumulated momentum (normalized),
            # which helps the optimizer punch through local optima -> stronger patch.
            with torch.no_grad():
