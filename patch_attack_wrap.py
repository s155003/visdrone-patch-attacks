...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_wrap"   # overridden per-variant
...
def apply_patch(img, patch, boxes_norm):
    ...
    if 'WRAP_MODE' in globals() and WRAP_MODE in ("tiles","contiguous") and globals().get('USE_WRAP', True):
        for (xc, yc, bw, bh) in boxes_norm:
            angle = estimate_car_angle(img, xc, yc, bw, bh, H, W)
            wrap_apply(out, img, patch, xc, yc, bw, bh, angle)
        return out
...
def run():
    ...
    if globals().get('USE_WRAP', False) and WRAP_MODE == "tiles":
        _n = WRAP_TILES
        patch = torch.rand(_n, 3, PATCH_PX, PATCH_PX, device=DEVICE, requires_grad=True)
    elif globals().get('USE_WRAP', False) and WRAP_MODE == "contiguous":
        patch = torch.rand(len(WRAP_ZONES), 3, PATCH_PX, PATCH_PX, device=DEVICE, requires_grad=True)
    else:
