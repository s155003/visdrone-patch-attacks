...
NUM_IMAGES = 30
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_scatter"   # overridden per-variant by the orchestrator
...
PATTERN_MODE = "shared"    # "shared" (all tiles same pattern) or "independent" (each its own)
...
def run():
    ...
    if PATTERN_MODE == "independent":
        patch = torch.rand(N_TILES, 3, PATCH_PX, PATCH_PX, requires_grad=True)  # each tile its own
    else:
        patch = torch.rand(3, PATCH_PX, PATCH_PX, requires_grad=True)           # shared
    ...
    pimg = (_pp.permute(1,2,0).numpy()*255).astype(np.uint8)[:,:,::-1]
