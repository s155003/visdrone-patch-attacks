...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_bodypanel"
...
def apply_patch(img, patch, boxes_norm):
    ...
    for (xc, yc, bw, bh) in boxes_norm:
        ...
        if FIXED_FRAC > 0:
            adapt_frac = FIXED_FRAC        # fixed size for every car
        else:
            # frac ramps from ~0.30 (tiny) up to ~0.55 (large)
        ...
        # estimate car orientation (radians of the LONG axis)
        angle = estimate_car_angle(img, xc, yc, bw, bh, H, W)
        # BODY-PANEL PLACEMENT: shift the patch center from the car center toward
        # ONE END along the car's long axis, so it lands on a hood/roof/trunk panel
        # and avoids the middle windshield/glass band.
        # car length ~ the longer box side; shift by END_OFFSET * that length.
        car_len = max(box_w_px, box_h_px)
        shift = END_OFFSET * car_len
        # direction of the long axis (unit vector) from the estimated angle
        import math as _m
        dx = _m.cos(angle); dy = _m.sin(angle)
        cx = int(xc*W + shift*dx); cy = int(yc*H + shift*dy)
        # HARD GLASS-CLEARANCE CLAMP (extreme prejudice): the patch's inner edge
        # must never cross into the middle glass band. The band spans
        # ±(GLASS_BAND/2)*car_len from the car center along the axis. The patch is
        # centered at END_OFFSET*car_len. So the patch half-size is capped so that
        #   (END_OFFSET*car_len) - (side/2) >= (GLASS_BAND/2)*car_len
        glass_half = (GLASS_BAND/2.0) * car_len
        patch_center_off = END_OFFSET * car_len
        # max half-size that still clears the glass band (with 1px safety margin)
        max_half_glass = max(2.0, patch_center_off - glass_half - 1.0)
        # also don't run off the far end of the car
        max_half_end   = max(2.0, (0.5*car_len) - patch_center_off)
        max_half = min(max_half_glass, max_half_end)
        side = int(min(max(pw, ph), 2*max_half))
        side = max(4, side)
...
def run():
    ...
    pimg = (final_patch.detach().cpu().permute(1,2,0).numpy()*255).astype(np.uint8)[:,:,::-1]
