...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_hsv"   # overridden
...
COLOR_SPACE = "hsv"          # "rgb" (optimize rgb directly) or "hsv" (optimize h,s,v)
...
def _hsv_to_rgb(hsv):
    """Differentiable HSV->RGB. hsv: [3,H,W] in [0,1]. Returns rgb [3,H,W] in [0,1]."""
    import torch as _t
    h,sat,val = hsv[0], hsv[1], hsv[2]
    h6=(h*6.0)%6.0; c=val*sat; x=c*(1-_t.abs((h6%2)-1)); m=val-c; z=_t.zeros_like(h)
    c0=((h6>=0)&(h6<1)).float();c1=((h6>=1)&(h6<2)).float();c2=((h6>=2)&(h6<3)).float()
    c3=((h6>=3)&(h6<4)).float();c4=((h6>=4)&(h6<5)).float();c5=((h6>=5)&(h6<6)).float()
    r=c*c0+x*c1+z*c2+z*c3+x*c4+c*c5
    g=x*c0+c*c1+c*c2+x*c3+z*c4+z*c5
    b=z*c0+z*c1+x*c2+c*c3+c*c4+x*c5
    return _t.stack([r+m,g+m,b+m],dim=0)
...
def paint_apply(out, img, patch, xc, yc, bw, bh, angle):
    ...
    # the car's oriented footprint inside this tight crop = the VALID pixels for statistics
    ...
    # PER-BOX: detect paint using ONLY this car's pixels for the statistics
