...
def apply_patch(img, patch, boxes_norm):
    ...
    for (xc, yc, bw, bh) in boxes_norm:
        # patch sized to the car's ACTUAL width and height separately (not a
        # square), each a fraction of the real box dims, centered on the car so
        # a small FRAC keeps it inside the car body (off the background corners).
