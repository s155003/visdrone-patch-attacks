"""
Evaluate a trained RetinaNet-ResNet50 on VisDrone test-dev: per-class AP@0.5 + AR.
Matches the YOLO eval methodology (IoU 0.5, 4-class summary car/van/truck/bus).

Set MODEL_PATH + FPN_MODE to match the trained model.
USAGE: cd ~/attack-test && ~/attack_env/bin/python 3_eval_retinanet.py
"""
import os, glob, json
...
# ==================== SETTINGS ====================
MODEL_PATH = "retinanet_custom.pt"     # or retinanet_custom.pt
FPN_MODE   = "custom"                    # MUST match how it was trained
TESTDEV    = Path(os.path.expanduser("~/attack-test/datasets/VisDrone_testdev"))  # 5-class labels
IMG_SIZE   = 640
DEVICE     = "cuda"
CONF       = 0.001
IOU_THRESH = 0.5
...
PAPER_4={0,1,2,3}  # car,van,truck,bus (0-indexed model classes)

def build_model(mode):
    if mode=="standard":
        bb=resnet_fpn_backbone("resnet50",weights=None,returned_layers=[2,3,4],extra_blocks=LastLevelP6P7(256,256))
        asz=((32,),(64,),(128,),(256,),(512,))
    else:
        bb=resnet_fpn_backbone("resnet50",weights=None,returned_layers=[1,2,3,4],extra_blocks=LastLevelMaxPool())
        asz=((16,),(32,),(64,),(128,),(256,))
    ag=AnchorGenerator(asz,((0.5,1.0,2.0),)*len(asz))
    return RetinaNet(bb,num_classes=NUM_CLASSES+1,anchor_generator=ag)

def load_gt(lbl, sx, sy):
    gts=[]
...
def compute_ap(recs, precs):
    # 11-point-ish: integrate precision over recall (area under PR)
    order=np.argsort(recs)
    recs=np.array(recs)[order]; precs=np.array(precs)[order]
...
def main():
    ...
    print(f"Evaluating {MODEL_PATH} ({FPN_MODE}) on {len(imgs)} test-dev images...")

    # collect per-class detections + gt counts
    per_cls={c:{"scores":[],"tp":[],"npos":0} for c in range(NUM_CLASSES)}
    for ip in imgs:
        orig=cv2.imread(ip)
        if orig is None: continue
        H0,W0=orig.shape[:2]; sx,sy=IMG_SIZE/W0,IMG_SIZE/H0
        lbl=str(TESTDEV/"labels"/(Path(ip).stem+".txt"))
        gts=load_gt(lbl,sx,sy)
        ...
        for i in order:
            ...
            cls=int(labels[i])-1   # model is 1-indexed (bg=0)
            ...
            is_tp = best_i>=IOU_THRESH and best_j not in used[cls]
            if is_tp: used[cls].add(best_j)
            per_cls[cls]["scores"].append(scores[i])
            per_cls[cls]["tp"].append(1 if is_tp else 0)
    ...
    for c in range(NUM_CLASSES):
        ...
        order=np.argsort(-np.array(d["scores"]))
        tp=np.array(d["tp"])[order]
        cum_tp=np.cumsum(tp); cum_fp=np.cumsum(1-tp)
        rec=cum_tp/npos; prec=cum_tp/(cum_tp+cum_fp)
        ap=compute_ap(list(rec),list(prec))
        final_recall=rec[-1] if len(rec) else 0.0
        print(f"{CLASS_NAMES[c]:10} {ap:8.3f} {final_recall:8.3f}")
        if c in PAPER_4: aps.append(ap); recs4.append(final_recall)
