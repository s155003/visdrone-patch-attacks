...
SAVE_DIR = "patch_examples_car2van_small"
OUT_DIR  = os.path.join(SAVE_DIR, "annotated")
IMG_SIZE = 640
IOU_THRESH = 0.5
SRC_IDX, TGT_IDX = 0, 1          # car (source), van (target)
PANELS = ["clean", "patched"]    # patch attack saves clean vs patched (not FGSM/PGD/C&W)

COLORS = {  # BGR for cv2
    "gt":   (0, 255, 255),   # yellow  - GT car box
    "van":  (0, 0, 255),     # red     - target detections
...
def draw_panel(img_bgr, dets, gt_boxes, label):
    ...
    for d in dets:
        ...
        if   cls == TGT_IDX: color, tag = COLORS["van"], f"van {conf:.2f}"
...
def main():
    ...
    pts = [p for p in sorted(glob.glob(os.path.join(SAVE_DIR, "*.pt")))
           if os.path.basename(p) != "universal_patch.pt"]   # exclude the learned patch tensor
    if not pts:
    ...
    print(f"  of which {len(flipped_first)} show at least one car->van flip (prefixed FLIP_).")
    if flipped_first:
