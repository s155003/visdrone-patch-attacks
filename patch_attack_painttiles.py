...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_painttiles"   # overridden
...
PAINT_TILES  = 49            # number of independent tiles to fill the detected paint area
...
def paint_apply(out, img, patch, xc, yc, bw, bh, angle):
    ...
    L = max(4.0, min(L, max(box_w_px,box_h_px)))*0.85
    Wd= max(4.0, min(Wd, max(box_w_px,box_h_px)))*0.85
...
def run():
    ...
    if globals().get('USE_PAINTTILES', False):
