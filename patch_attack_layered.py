...
NUM_IMAGES = 100
IMG_SIZE   = 640
SAVE_DIR   = "patch_examples_layered"   # overridden
...
COMBINE_METHOD = "union"     # use the best (largest-area) mask from the combo results
LAYER_MODE  = "additive"    # "additive" (sum layers), "multiscale", "blend", or "none"
N_LAYERS    = 3             # number of stacked adversarial layers on the same on-car area
...
def _detect_paint_mask(crop_bgr):
    """Combine brightness + dom-color per COMBINE_METHOD. 1=paint, 0=glass/wheel/ground."""
    import numpy as _np, cv2 as _cv2
    if crop_bgr.shape[0]<3 or crop_bgr.shape[1]<3:
        return _np.ones(crop_bgr.shape[:2],_np.float32)
    mb=_mask_brightness(crop_bgr); md=_mask_domcolor(crop_bgr)
    if COMBINE_METHOD=="adaptive":
        # light cars -> brightness works best; dark cars -> dom-color works best
        avg=_cv2.cvtColor(crop_bgr,_cv2.COLOR_BGR2GRAY).mean()
        m = mb if avg>=110 else md      # threshold ~ mid-gray
...
def paint_apply(out, img, patch, xc, yc, bw, bh, angle):
    ...
    # LAYERING: combine N stacked adversarial layers into one effective pattern.
    if patch.dim() == 4:
        import torch as _t
        mode = globals().get('LAYER_MODE', 'additive')
        if mode == 'additive':
            # sum all layers then clamp to valid range -> more signal accumulated on same area
            psrc = patch.sum(dim=0).clamp(0, 1)
        elif mode == 'multiscale':
            # each layer at a different spatial scale (coarse->fine), then average
            acc = 0.0
            for li in range(patch.shape[0]):
                sc = 2 ** li   # layer 0 full-res, layer 1 half, layer 2 quarter (coarser)
                ph2 = max(2, PATCH_PX // sc)
                lo = _t.nn.functional.interpolate(patch[li:li+1], size=(ph2, ph2), mode='bilinear', align_corners=False)
                up = _t.nn.functional.interpolate(lo, size=(PATCH_PX, PATCH_PX), mode='bilinear', align_corners=False)[0]
                acc = acc + up
            psrc = (acc / patch.shape[0]).clamp(0, 1)
        elif mode == 'blend':
            # alpha-blend layers equally
            psrc = patch.mean(dim=0).clamp(0, 1)
        else:
            psrc = patch[0].clamp(0, 1)
    else:
...
def run():
    ...
    if globals().get('LAYER_MODE','none') != 'none' and globals().get('USE_PAINT', False):
        patch = torch.rand(N_LAYERS, 3, PATCH_PX, PATCH_PX, device=DEVICE, requires_grad=True)
    else:
